...
namespace HOOKS
{
	...
	{ 
		...
		if (INTERFACES::Engine->IsConnected() && INTERFACES::Engine->IsInGame())
		{
			...
			if (SETTINGS::settings.fakewalk_bool)
				fakewalk->do_fakewalk(cmd);
	...
	void __stdcall HookedPaintTraverse(int VGUIPanel, bool ForceRepaint, bool AllowForce)
	{
		...
		if (panel_name == "MatSystemTopPanel")
		{
			...
			auto DrawInject = [screen_width, screen_height]()
			{
				...
				{
					static constexpr char thulium_text[] = "T h u l i u  m      ";
					...
					for (int i = 0; i < text_len; i++)
					{
						...
						if (i == current_letter - 1 || i == current_letter - 2)
						{
							...
							RENDER::DrawF(cur_width + (frac_2 * intro_max_width_offset), center_height + (frac_2 * intro_max_height_offset), FONTS::welcome_font, false, true, CColor(6, 137, 255, alpha), std::string(1, thulium_text[i]));
						}
						else
							RENDER::DrawF(cur_width, center_height, FONTS::welcome_font, false, true, CColor(6, 137, 255, alpha), std::string(1, thulium_text[i]));
					}
				}
			};
