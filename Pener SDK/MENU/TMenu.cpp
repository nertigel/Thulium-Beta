...
namespace TGUI
{
	namespace MENU
	{
		void Do()
		{
			...
			TGUI_BeginWindow("Thulium Beta", HELPERS::TGUI_Vector2D(200, 200), HELPERS::TGUI_Vector2D(800, 650), 2);
			{
				static char aim_mode[][TGUI_MAX_STRING_LENGTH] = { "rage", "legit" };
				static char acc_mode[][TGUI_MAX_STRING_LENGTH] = { "head", "upper body", "body", "hit-scan" };
				...
				case 0: {
					...
					if (SETTINGS::settings.aim_bool)
					{
						...
						if (SETTINGS::settings.aim_type == 0)
						{
							...
							{
								TGUI_Combobox("hitbox", 4, acc_mode, SETTINGS::settings.acc_type);
							...
							TGUI_BeginGroupbox("corrections", AutoCalc(7));
							{
								...
								TGUI_SameLine();
								TGUI_Keybind("", SETTINGS::settings.quickstopkey);
								//TGUI_Combobox("auto stop method", autostop_method, SETTINGS::settings.autostopmethod);
								TGUI_Checkbox("angle correction", SETTINGS::settings.resolve_bool); TGUI_ToolTip("Correct enemy's fake angles.");
								TGUI_Checkbox("override enable", SETTINGS::settings.overrideenable); TGUI_ToolTip("Override enemy's angles.");
								TGUI_SameLine();
								TGUI_Keybind("override key", SETTINGS::settings.overridekey);
								TGUI_Combobox("override method", 2, override_method, SETTINGS::settings.overridemethod);
								TGUI_Combobox("delay shot", 2, delay_shot, SETTINGS::settings.delay_shot);
							}
							TGUI_EndGroupbox();
						...
						else
						{
							TGUI_BeginGroupbox("trigger and others", AutoCalc(3));
							{
								...
								TGUI_Checkbox("position adjustment", SETTINGS::settings.back_bool);
							}
							TGUI_EndGroupbox();
						}
					}
				...
				case 1: {
					...
					if (SETTINGS::settings.esp_bool)
					{
						...
						TGUI_BeginGroupbox("chams", AutoCalc(3));
						{
							...
							TGUI_Combobox("model type", 2, chams_type, SETTINGS::settings.chamstype);
						}
						TGUI_EndGroupbox();
						...
						TGUI_BeginGroupbox("world", AutoCalc(11));
						{
							...
							TGUI_Slider("modulate", "%", 0, 100, SETTINGS::settings.daytimevalue, 0); TGUI_ToolTip("Low modulate value will cause nightmode.");
						...
						TGUI_BeginGroupbox("local player", AutoCalc(5));
						{
							...
							TGUI_Checkbox("lby indicator", SETTINGS::settings.lbyenable);
						}
						TGUI_EndGroupbox();
					}
				} break;
				...
				case 2: {
					...
					{
						TGUI_BeginGroupbox("movement", AutoCalc(5));
						{
							...
							TGUI_Checkbox("fake-walk", SETTINGS::settings.fakewalk_bool); TGUI_AttachToPreviousWidget(); TGUI_Keybind("bind##fake-walk", SETTINGS::settings.fakewalk_key);
						}
						TGUI_EndGroupbox();
						...
						TGUI_BeginGroupbox("clantag changer", AutoCalc(3));
						{
							...
							TGUI_TextInput("name", SETTINGS::settings.misc_clantag_text);
						...
						TGUI_BeginGroupbox("extra", AutoCalc(1));
						{
				...
				case 3: {
					...
					TGUI_BeginGroupbox("configuration", 90);
					{
						...
						if (TGUI_Button("Load Config"))
						{
							...
							log.color_line.PushBack("[Thulium] ", LIGHTBLUE);
							log.color_line.PushBack("Configuration loaded.", CColor(200, 200, 200));

							in_game_logger.AddLog(log);

							//INTERFACES::cvar->ConsoleColorPrintf(CColor(175, 200, 5), "[Thulium] ");
